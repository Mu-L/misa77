...
#pragma GCC unroll hashtab_wid
            ...
            if (match_len >= min_match_len)
            {
                for (uint64_t npos = pos + 1;
                     npos <= pos + lookahead and npos + max_match_len <= match_end_limit;
                     npos++)
                {
                    uint64_t nlst = 0;
...
#pragma GCC unroll hashtab_wid
                    ...
                    pos = (nmatch_len > match_len ? npos : pos);
                    lst = (nmatch_len > match_len ? nlst : lst);
                    match_len = (nmatch_len > match_len ? nmatch_len : match_len);
                }
